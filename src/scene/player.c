...
void playerAttackOverlap(struct DynamicSceneOverlap* overlap) {
    ...
    if ((overlap->otherEntry->flags & DynamicSceneEntryHasTeam) != 0 && player->attackInfo) {
        ...
        soundPlayerPlay(SOUNDS_PUNCHIMPACT, 1.0f, 0, &player->transform.position);
    }
}
...
void playerStartAttack(struct Player* player) {
    if (!player->attackCollider) {
        ...
        soundPlayerPlay(soundListRandom(&gTeamFactions[player->playerIndex]->playerSounds.attackSounds), 1.0f, 0, &player->transform.position);
...
void playerEnterDeadState(struct Player* player) {
    ...
    playerEndAttack(player);
    soundPlayerPlay(soundListRandom(&gTeamFactions[player->playerIndex]->playerSounds.deathSounds), 1.0f, 0, &player->transform.position);
...
void playerEnterJumpState(struct Player* player) {
    ...
    player->animationSpeed = 1.0f;
    soundPlayerPlay(soundListRandom(&gTeamFactions[player->playerIndex]->playerSounds.jumpSounds), 1.0f, 0, &player->transform.position);
...
void playerStateWalk(struct Player* player, struct PlayerInput* input) {
    ...
    if (isMoving != hasWalkingSound) {
        if (isMoving) {
            player->walkSoundEffect = soundPlayerPlay(soundListRandom(&gTeamFactions[player->playerIndex]->playerSounds.walkSounds), 1.0f, 0, &player->transform.position);
...
void playerApplyDamage(struct Player* player, float amount, struct Vector3* origin, float knockback) {
    if (player->transform.position.y < INVINCIBLE_JUMP_HEIGHT || player->state == playerStateSpawn) {
        ...
        if (damageHandlerApplyDamage(&player->damageHandler, amount, PLAYER_INVINCIBILITY_TIME)) {
            soundPlayerPlay(soundListRandom(&gTeamFactions[player->playerIndex]->playerSounds.damageSounds), 1.0f, 0, &player->transform.position);
